...
///////////////////////////////////////////////////////////////////////
     ...
     int nsob,nparam(4);
     double *xsob;
     int sob_flag(0); //use sobseq rather than random points
     long idums;
...
p4range=(p4max-p4min);
 ...
 if(like_calc_flag==1){
   ...
   double p1min0, p2min0, p3min0, p4min0;
   cout<<"start: "<<p1min<<"\t"<<p1max<<"\t"<<p2min<<"\t"<<p2max<<endl;
   p1min0=p1min;
   p2min0=p2min;
   p3min0=p3min;
   p4min0=p4min;
   ...
   //random sample paramater space to find limits of integration
   for(i=1;i<=200;i++){
       p1=p1min0+dran2(&idums)*p1range;
       p2=p2min0+dran2(&idums)*p2range;
       p3=p3min0+dran2(&idums)*p3range;
       p4=p4min0+dran2(&idums)*p4range;
     ...
     if(like>1.0e-10){
       ...
       cout<<p1min<<"\t"<<p1max<<"\t"<<p2min<<"\t"<<p2max<<"\t"<<like<<endl;
     }
   }
