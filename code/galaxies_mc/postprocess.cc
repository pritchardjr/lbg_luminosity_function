...
///////////////////////////////////////////////////////////////////////
     ...
     npoints=n1*n2*4;
     npoints =32000;  //memory limitations start to apply after this
...
//marginalised parameter pdfs
    ...
    */
	 ...
	 */
       /////////////////////////////////////////////////
       //calculate S/N contours
       /////////////////////////////////////////////////
	 
	 //use precalculated table to create 2D spline
	 Spline2D flucSp;
	 file="./fluc_table_short_spline.dat";
	 flucSp.loadFileSpline(file);
	 double delta;
	 double k(0.1);

       //calculate xi pdf at given redshift
    
    files=data_dir+"sn_contour.dat";
    fout_xi.open(files.c_str());
    vector<double> snv;
    double sn;
    ...
    while(z<18.0){
       ...
       while(!fin.eof()){
          ...
          fin>>xi;
          xv.push_back(xi);
	  //calculate delta_{T_b} corresponding to this x_i value
	  if(xi>1.0) xi=1.0;
	  sn=flucSp.returnValue(z,xi)*pow(tocm.tBrightSat(z,xi)*1e3,2.0)*pow(k,3.0)/2.0/PI/PI;
	  sn=sqrt(sn);
	  //cout<<z<<"\t"<<xi<<"\t"<<sn<<endl;
	  snv.push_back(sn);
          getline(fin,str);
       ...
       for(i=0;i<ndata;i++){
          ibin=floor((snv[i]-xmin)/dx);
          pdf[ibin]+=likev[i]/likeTot/dx;
       }
    
       files=data_dir+"sn_pdf_z"+numToString(z)+".dat";
       fout.open(files.c_str());
       pdftot=0.0;
       for(i=0;i<nbins;i++){
	 pdftot+=pdf[i]*dx;
	 ...
	 //cumulative pdf
	 //pTB=rz.getPowerFZH(z,k,xmin+i*dx);  //fix xi=fcoll*zeta
	 fout_xi<<z<<"\t"<<xmin+i*dx<<"\t"<<pdftot<<"\t"<<(xmin+i*dx)/tocm.tSky(z)<<endl;
       }
       fout.close();
    ...
    fout_xi.close();
       ...
       /////////////////////////////////////////////////
       //calculate d x_i/dz  contours
       /////////////////////////////////////////////////
   	
       //calculate xi pdf at given redshift
    
    files=data_dir+"dxdz_contour.dat";
    fout_xi.open(files.c_str());
    vector<double> dxdzv;
    double dxdz, xim, xip;
    ...
    while(z<18.0){
	  ...
	  dxdzv.push_back(dxdz);
       ...
       for(i=0;i<ndata;i++){
          ibin=floor((dxdzv[i]-xmin)/dx);
          if(ibin<nbins) pdf[ibin]+=likev[i]/likeTot/dx;
       }
    
       files=data_dir+"dxdz_pdf_z"+numToString(z)+".dat";
       fout.open(files.c_str());
       pdftot=0.0;
       for(i=0;i<nbins;i++){
	 pdftot+=pdf[i]*dx;
