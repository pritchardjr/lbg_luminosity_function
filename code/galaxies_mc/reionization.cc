...
double Reionization::nmBub(double mb, double z)
{
  ...
  if(count==0 || fabs(z-zsave)>1.0e-4 || fabs(zetaBub-zetaSave)>1.0e-6){
    zsave=z;
    ...
    if(Q!=0.0) renorm=getZeta()*c->fColl(z)/Q;
  ...
  if(B0<1.0e-20){
    ...
    //cout<<z<<"\t"<<getZeta()<<"\t"<<deltac<<"\t"<<K<<"\t"<<sigMmin<<"\t"<<B0<<endl;
    //return 0.0;  //at low z B0 can turn negative indicating x_i=1.0
    B0=0.0;
  }
...
double Reionization::getCorrXD(double z, double rvec[], double xi[], int nr)
{
  ...
  if(xi[1]<0.0){
    xion=c->fColl(z)*getZeta();
    cout<<xion<<"\t"<<getZeta()<<"\t"<<c->fColl(z)<<endl;
  }else{
    xion=a->getXI(z);
  }
...
double Reionization::getCorrTB(double z, double rvec[], double xi[], int nr, double corrDD[], double corrXD[], double corrXX[], double xH)
{
  ...
  for(i=1;i<=nr;i++){
    ...
    xi[i]=corr/xH/xH;
    //cout<<"TB \t"<<rvec[i]<<"\t"<<xi[i]<<"\t"<<corrXX[i]<<"\t"<<corrXD[i]<<"\t"<<corrDD[i]<<endl;
  }
...
///////////////////////////////////////////////////////////////////////
//  Calculate FZH power spectrum at single wavenumber
///////////////////////////////////////////////////////////////////////
double Reionization::getPowerFZH(double z, double k, double xi)
{
  double pFZH;
  int nr(79), nk(1);
  double *rN, *kN;
  ...
  //outside of parameter space give the linear power spectrum
  //return Plinear(k,z,c)*xH*xH; 
  //setZeta(5.0);
  rN=dvector(1,nr);
  kN=dvector(1,nk);
  getRVec(rN);
  ...
  //cout<<fillingQ(z)<<endl;

  getCorrDD(z,rN,corrDDV,nr);
  cout<<"XD...";
  //getCorrXD(z,rN,corrXDV,nr);
  for(i=1;i<=nr;i++) corrXDV[i]=0.0;
  cout<<"XX...";
  getCorrXX(z,rN,corrXXV,nr);
  //  FTTable(corrXXV,rN,nr,powerXXV,kN,nk);
  cout<<"TB...";
  getCorrTB(z,rN,corrTBV,nr,corrDDV,corrXDV,corrXXV,xH);
  if(xi<0.01){
    FTTable(corrDDV,rN,nr,powerTBV,kN,nk);    
  }else{
  ...
  free_dvector(rN,1,nr);
  free_dvector(kN,1,nk);

  free_dvector(corrDDV,1,nr);
  free_dvector(corrXDV,1,nr);
  free_dvector(corrXXV,1,nr);
  free_dvector(corrTBV,1,nr);

  free_dvector(powerDDV,1,nk);
  free_dvector(powerXDV,1,nk);
  free_dvector(powerXXV,1,nk);
  free_dvector(powerTBV,1,nk);

  //xidd(0.0,z,c,2);
  //cout<<getZeta()<<"\t"<<Plinear(k,z,c)<<endl;
  return pFZH*xH*xH;
}
...
double Reionization::clumpingBub(double z, double xi)
{
  ...
  setZeta(xi/c->fColl(z));
  if(getZeta()<1.0) setZeta(1.01);  //fudge to avoid problems
  ...
  //test for full ionization
  //NOT SURE IF THIS IS BEST WAY TO IMPLEMENT THIS
  if(nmBub(mMin,z)<1.0e-20){
    //clump=a->getClumpingMEHR(a->deltaIMEHR(z,1.0-1e-4),z);
    clump=a->getClumpingMEHR(exp(13.9),z);
	 return clump;
  }
